  ...
  if (req.user) {
    User.findOne({ linkedin: profile.id }, function(err, existingUser) {
      if (existingUser) {
        req.flash('errors', { msg: 'There is already a LinkedIn account that belongs to you. Sign in with that account or delete it, then link it with your current account.' });
          console.log("There is already a LinkedIn account that belongs to you. Sign in with that account or delete it, then link it with your current account.");
        done(err);
      } else {
        User.findById(req.user.id, function(err, user) {
    ...
    User.findOne({ linkedin: profile.id }, function(err, existingUser) {
      ...
      User.findOne({ email: profile._json.emailAddress }, function(err, existingEmailUser) {
        ...
        console.log("There is already an account using this email address. Sign in to that account and link it with LinkedIn manually from Account Settings.");
          done(err);
        } else {
          var user = new User();
          user.linkedin = profile.id;
...
exports.isAuthenticated = function(req, res, next) {
  if (req.isAuthenticated()) res.redirect("/nope");
  res.redirect("/");
};
